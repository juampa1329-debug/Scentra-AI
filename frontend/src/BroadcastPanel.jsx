      ...
      {activeModule === "message_template" ? (
        <div className="mass-template-layout">
          <article className="panel glass-card module-card">
            <div className="panel-head"><h2>Message Template</h2><span>{bodyChars} / 1024</span></div>
            <div className="form-grid two">
              <label>Template name *<input value={metaForm.name} onChange={(event) => setMetaForm((prev) => ({ ...prev, name: event.target.value }))} placeholder="system_order_success_notification" /></label>
              <label>Locale *<select value={metaForm.language} onChange={(event) => setMetaForm((prev) => ({ ...prev, language: event.target.value }))}>{LOCALES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
            </div>
            ...
            <label>Header type *<select value={metaForm.header_type} onChange={(event) => setMetaForm((prev) => ({ ...prev, header_type: event.target.value }))}>{HEADER_TYPES.map(([value, label]) => <option key={value || "none"} value={value}>{label}</option>)}</select></label>
            {metaForm.header_type === "TEXT" ? <label>Header text<input value={metaForm.header_text} onChange={(event) => setMetaForm((prev) => ({ ...prev, header_text: event.target.value }))} maxLength={60} /></label> : null}
            {["IMAGE", "VIDEO", "DOCUMENT"].includes(metaForm.header_type) ? <label>Header media handle<input value={metaForm.header_media_handle} onChange={(event) => setMetaForm((prev) => ({ ...prev, header_media_handle: event.target.value }))} placeholder="Handle de Meta" /></label> : null}
            <div className="template-tools"><button type="button" onClick={() => insertToken("customer_name")}>Name</button><button type="button" onClick={() => insertToken("customer_city")}>Ciudad</button><button type="button" onClick={() => insertToken("business_name")}>Empresa</button><button type="button" onClick={rewriteBody}>AI Re-write</button><EmojiButton onPick={addEmoji} /></div>
            <label>Message body *<textarea ref={bodyRef} rows={8} maxLength={1024} value={metaForm.body_text} onChange={(event) => setMetaForm((prev) => ({ ...prev, body_text: event.target.value }))} placeholder="Hola, {{customer_name}}, ..." /></label>
            <label>Footer text<input value={metaForm.footer_text} onChange={(event) => setMetaForm((prev) => ({ ...prev, footer_text: event.target.value }))} maxLength={60} placeholder="Atentamente, Equipo Scentra" /></label>
            <div className="template-tools"><button type="button" onClick={() => addButton("QUICK_REPLY")}>+ Quick reply</button><button type="button" onClick={() => addButton("URL")}>+ URL</button><button type="button" onClick={() => addButton("PHONE_NUMBER")}>+ Phone</button></div>
            <div className="button-editor-list">{(metaForm.buttons || []).map((button, idx) => <div key={`${button.type}-${idx}`}><select value={button.type} onChange={(event) => updateButton(idx, emptyButton(event.target.value))}><option value="QUICK_REPLY">Quick Reply</option><option value="URL">URL</option><option value="PHONE_NUMBER">Phone</option></select><input value={button.text} onChange={(event) => updateButton(idx, { text: event.target.value })} placeholder="Button text" />{button.type === "URL" ? <input value={button.url} onChange={(event) => updateButton(idx, { url: event.target.value })} placeholder="https://..." /> : null}{button.type === "PHONE_NUMBER" ? <input value={button.phone_number} onChange={(event) => updateButton(idx, { phone_number: event.target.value })} placeholder="+573..." /> : null}<button type="button" onClick={() => removeButton(idx)}>x</button></div>)}</div>
