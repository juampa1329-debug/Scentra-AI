...
function App() {
  ...
  const [integrationForm, setIntegrationForm] = useState({ provider: "meta", channel: "whatsapp", status: "connected", dispatch_mode: "stub", phone_number_id: "", business_account_id: "", graph_api_version: "v22.0", access_token_env: "SCENTRA_META_ACCESS_TOKEN" });
  ...
  const loadDashboard = async (silent = false) => {
    if (!accessToken) return;
    try {
      const [overviewData, inboxData, integrationData, endpointData, eventData, plansData] = await Promise.all([
        apiCall("/saas/v1/billing/overview"), apiCall("/saas/v1/conversations?limit=100"), apiCall("/saas/v1/integrations"),
        apiCall("/saas/v1/webhooks/endpoints"), apiCall("/saas/v1/webhooks/events?limit=20"), apiCall("/saas/v1/billing/plans"),
      ]);
      setBillingOverview(overviewData); setConversations(inboxData?.conversations || []); setIntegrations(integrationData || []);
  ...
  const saveIntegration = async (event) => {
    ...
    try {
      await apiCall("/saas/v1/integrations", { method: "POST", body: JSON.stringify({ provider: integrationForm.provider, channel: integrationForm.channel, status: integrationForm.status, secret_ref: dispatchMode === "stub" ? "" : `env:${accessTokenEnv}`, config_json: { dispatch_mode: dispatchMode, phone_number_id: phoneNumberId, business_account_id: (integrationForm.business_account_id || "").trim(), graph_api_version: (integrationForm.graph_api_version || "v22.0").trim(), access_token_env: accessTokenEnv } }) });
        ...
        {activeView === "dashboard" ? (
          <section className="dashboard-page">
            ...
            <div className="metric-grid">
              ...
              <article className="metric-card rose"><span>Integraciones</span><strong>{number(connectedIntegrations)} / {number(billingLimits.max_integrations)}</strong><small>Canales</small></article>
              <article className="metric-card violet"><span>Webhooks activos</span><strong>{number(activeWebhooks)}</strong><small>Endpoints</small></article>
            </div>
            <section className="dashboard-layout">
              <article className="panel glass-card wide-panel"><div className="panel-head"><h2>Funnel comercial</h2><span>Conversion por etapa</span></div>{[["Contactados",100,conversations.length || 1],["Interes",42,unreadTotal],["Intencion de compra",24,Math.round(conversations.length * 0.24)],["Pago pendiente",8,0],["Pago confirmado",4,0]].map(([label,width,value]) => <div className="funnel-line" key={label}><div><span>{label}</span><small>{value}</small></div><div className="meter"><span style={{ width: `${width}%` }} /></div></div>)}</article>
              ...
              <article className="panel glass-card chart-panel"><div className="panel-head"><h2>Actividad reciente</h2><span>eventos + mensajes</span></div><svg viewBox="0 0 640 220" role="img" aria-label="Grafica de actividad"><defs><linearGradient id="lineGlow" x1="0" x2="1"><stop stopColor="#5eead4" /><stop offset="1" stopColor="#60a5fa" /></linearGradient></defs><path d="M20 178 C120 174 140 160 210 166 C290 174 300 64 356 72 C420 80 410 170 500 142 C562 124 578 76 620 62" fill="none" stroke="url(#lineGlow)" strokeWidth="8" strokeLinecap="round" /><path d="M20 178 C120 174 140 160 210 166 C290 174 300 64 356 72 C420 80 410 170 500 142 C562 124 578 76 620 62 L620 210 L20 210 Z" fill="rgba(94,234,212,0.13)" /></svg></article>
              <article className="panel glass-card"><div className="panel-head"><h2>Canales</h2><span>setup</span></div><div className="channel-list"><div><strong>{number(connectedIntegrations)}</strong><span>Integraciones</span></div><div><strong>{number(activeWebhooks)}</strong><span>Webhooks</span></div><div><strong>{number(conversations.filter((item) => item.takeover).length)}</strong><span>Chats en humano</span></div></div></article>
            </section>
          </section>
        ) : activeView === "inbox" ? (
